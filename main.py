...
START_COUNTER = 0
GAME_STATE_WON = "You are the winner! The correct word was: {word}"
...
def start_game_round():
    while True:
        ...
        game_state["masked_word"] = [
            UNKNOWN_STATE_CHAR for _ in range(len(game_state["random_word"]))]
...
def check_game_state(game_state) -> str:
    if game_state["attempts_counter"] == LAST_HANGMAN_ATTEMPT:
        ...
        return GAME_STATE_WON.format(word=game_state["random_word"].upper())
    ...
    while True:
        ...
        if len(player_guess) != 1 or not player_guess.isalpha():
            ...
            print(f"You've already guessed '{
                  player_guess}'. Try a different letter")
            continue
...
def check_player_guess(game_state, player_guess: str):
    if player_guess in game_state["random_word"]:
        ...
        attempt_countdown = LAST_HANGMAN_ATTEMPT - \
            game_state["attempts_counter"]
        print(f"\nOops, you gave a wrong letter. Attempts remaining: {
              attempt_countdown}")
...
def print_state(game_state):
    ...
    if game_state["guessed_letters"]:
        print(f"That's your guesses so far: {
